...
function extend(Route) {
	...
	return class RoutePath extends Route {
		...
		initRoute(app) {
			...
			if (pathPart == null) throw this.debugError('[routerPath.PATH_PART] must be set on a path route');
			if (!isString(pathPart)) throw this.debugError('[routerPath.PATH_PART] must be a string');
		}
