...
const pathPlugin = new Plugin(
	pkg,
	{symbols: ['PATH', 'PATH_PART', 'GET_PATH_PART', 'PARAMS']},
	extend
);

module.exports = pathPlugin;

const {PATH, PATH_PART, GET_PATH_PART, PARAMS} = pathPlugin;
...
function extend(Route) {
	...
	return class PathRoute extends Route {
		...
		/**
		 * Determine path part.
		 * This has to happen before calling superior `[INIT_ROUTE]`, as otherwise
		 * `plugin-ordered` will do ordering before `[PATH_PART]` is defined
		 * if it's defined from name or by `[GET_PATH_PART]`.
		...
		[INIT_ROUTE]() {
			...
			// Call superior (including ordering)
			super[INIT_ROUTE]();
		}
		...
		[HANDLE_MATCH](req, match) {
			...
			const pathOld = req[PATH];
			req[PATH] = (pathOld === undefined ? req.url : pathOld)
