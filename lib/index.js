...
const Plugin = require('@overlook/plugin'),
	{INIT_PROPS, INIT_ROUTE, DEBUG_ERROR} = require('@overlook/route'),
...
const pathPlugin = new Plugin(
	pkg,
	{symbols: ['PATH_PART', 'GET_PATH_PART', 'URL_PATH', 'PARAMS', 'PATH_UNCONSUMED']},
	[pluginMatch, pluginOrder, pluginOrdered],
	(Route, {
		PATH_PART, GET_PATH_PART, URL_PATH, PARAMS, PATH_UNCONSUMED,
		...
		async [INIT_ROUTE]() {
			...
			const parentPathRoute = findParent(this, route => hasOwnProp(route, PATH_PART));

			// Inherit path part from name (or '' if this is top path route)
			let pathPart = this[PATH_PART];
			if (pathPart === undefined) {
				pathPart = this[GET_PATH_PART]();
				if (pathPart === undefined) pathPart = parentPathRoute ? this.name : '';
			...
			// Construct URL path
			let urlPath = parentPathRoute ? parentPathRoute[URL_PATH] : '';
			if (urlPath !== '/') urlPath += '/';
			urlPath += pathPart;
